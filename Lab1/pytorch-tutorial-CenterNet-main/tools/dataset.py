...
class VOC(Dataset):
    ...
    def __init__(self, split, transform=None, image_size=(384, 384), len_tiny=500,
        ...
        if not os.path.exists(os.path.join(data_dir, "train_images.json")) or not os.path.exists(os.path.join(data_dir, "test_images.json")):
            print("Raw dataset has not been parsed. Execute tools/parse_raw_dataset.py first. Exitting")
            print(os.path.join(data_dir, "test_images.json"))
            print(os.path.join(data_dir, "train_images.json"))
            exit(1)
        ...
        with open(os.path.join(data_dir, "{}_annotations.json".format(_split)), 'r') as f:
            self.anns = json.load(f)  # TODO
            if self.split == 'tiny':
                self.anns = self.anns[:len_tiny]  # TODO: keep the first len_tiny elements in self.anns
    ...
    def __len__(self):
        return len(self.images_path)  # TODO: return the number of samples in this dataset, this is the length of either self.images_path
    ...
    def __getitem__(self, item_idx):
        ...
        labels = self.anns[item_idx]['labels']  # list (nBoxes), each label is int - TODO: similar to `boxes`, extract labels
        ...
        offset_map = np.zeros((2, self.output_size[0], self.output_size[1]))  # TODO: similar to heat_map, initialize offset_map
        # TODO cont: remember the order of dimensions is: Channel, Height, Width
        size_map = np.zeros((2, self.output_size[0], self.output_size[1]))  # TODO: similar to heat_map, initialize size_map

        # fill those 3 maps above using elements in `boxes` and `labels`
        for box, obj_type in zip(boxes, labels):
            box_to_label(box, obj_type, heat_map, offset_map, size_map) # TODO: provide input for box_to_label()
