...
def create_data_lists(dataset_path, model_input_size=(384, 384)):
    ...
    for dataset in subset:
        ...
        for image in images_list:
            ...
            annotation = parse_annotation(os.path.join(dataset_path, dataset, 'Annotations', image+'.xml'),model_input_size)
    ...
    nObjects = 0
    # get the list of trainval images
    with open(os.path.join(dataset_path, 'VOCtest_06-Nov-2007/VOC2007/ImageSets/Main/test.txt'), 'r') as f:  # TODO
        images_list = f.readlines()
        # clean up images' names by removing the trailing '\n'
    images_list = [im.strip() for im in images_list]
    images_list.sort()
    ...
    # for each image in images_list, create its full path & parse its annotation
    for image in images_list:
        image_path = os.path.join(os.path.join(dataset_path, 'VOCtest_06-Nov-2007', 'VOC2007', 'JPEGImages', image + '.jpg'))  # TODO
        annotation = parse_annotation(os.path.join(dataset_path, 'VOCtest_06-Nov-2007', 'VOC2007', 'Annotations', image + '.xml'),
                                      model_input_size)  # TODO
        nObjects += len(annotation['boxes'])  # TODO
        # store image_path & annotation in their corresponding lists
        test_image_path.append(image_path)  # TODO
        test_annotations.append(annotation)  # TODO
