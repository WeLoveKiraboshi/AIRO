...
def box_compute_cornerNet_radius(box, min_iou=0.3):
    ...
    r3 = (b3 + sq3) / 2
    # TODO: compute `r` as defined in paragraph Computing \sigma_p of Section 2.1.2
    r = min(r1, r2, r3)
    return r
...
def draw_gaussian_on_matrix(box, mat_to_draw_onto, model_downsample_factor=4.0, min_iou=0.3, gaussian_limit=3.0,
    ...
    if 'center' not in kwargs:
        center_x = (box[2] + box[0])/2 # TODO: find x-coordiante of box's center
        center_y = (box[3] + box[1])/2  # TODO: find y-coordiante of box's center
    ...
    diff_square_gauss = (pts_in_gaussian - p_tilde)**2  # (n_pts, 2) each row px - ptilde_x, px - ptilde
    prob = np.exp(-np.sum(diff_square_gauss, axis=1)/(2*sigma**2))
    # print(np.sum(diff_square_gauss,axis=1).shape)
...
def box_to_label(box, obj_type, heat_map, offset_map, size_map,
    ...
    draw_gaussian_on_matrix(box, heat_map[obj_type])  # TODO

    # TODO: get center location in the original image (from the box) and in the low resolution image
    center_x = box[0]+(box[2]-box[0])/2 # TODO
    center_y = box[1]+(box[3]-box[1])/2  # TODO
    center = np.array([center_x, center_y])  # shape (2)
    center_low_res = np.floor(center/model_downsample_factor).astype(int)  # shape (2) - TODO: compute p_tilde
    ...
    size_map[0, center_low_res[1], center_low_res[0]] = w  # TODO: similar to offset_map use `center_low_res` to set the value for center pixel in size map
    size_map[1, center_low_res[1], center_low_res[0]] = h  # TODO: similar to offset_map use `center_low_res` to set the value for center pixel in size map
...
#     # TODO: invoke draw_gaussian_on_matrix on the right channel of heat_map to splash this box's center onto it
#
#     draw_gaussian_on_matrix(box, heat_map[obj_type, :, :], model_downsample_factor, min_iou, gaussian_limit)  # TODO
#
#
#     # TODO: get center location in the original image (from the box) and in the low resolution image
#     center_x = (box[2] + box[0]) / 2  # TODO: find x-coordiante of box's center
...
#     # TODO: compute the center offset according to Equation.3
#     # TODO cont: result is stored in `offset` which is a numpy.array of shape (2) made of (offset_x, offset_y)
#     offset = center/model_downsample_factor - center_low_res  # shape (2)
#
#     assert 0.0 <= offset[0] < 1.0 and 0.0 <= offset[1] < 1.0, \
#         "Expected offset must be in [0, 1), get {}".format(offset)
#     offset_map[0, center_low_res[1], center_low_res[0]] = offset[0]  # x-offset
...
#     # TODO: compute size of this box on the low-res image according to Equation.2
#     # Note: remember to divide them for model_downsample_factor
#     w, h = center_low_res[0], center_low_res[1]
#     assert w > 0 and h > 0, "Negative size, w = {}, h = {}".format(w, h)
#     size_map[0, center_low_res[1], center_low_res[0]] = w/model_downsample_factor  # TODO: similar to offset_map use `center_low_res` to set the value for center pixel in size map
#     size_map[1, center_low_res[1], center_low_res[0]] = h/model_downsample_factor  # TODO: similar to offset_map use `center_low_res` to set the value for center pixel in size map
