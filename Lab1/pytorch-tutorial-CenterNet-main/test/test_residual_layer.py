...
model_file = 'ctdet_pascal_resdcn18_384.pth' #'voc07_prediction_for_images_09-13-15-39.pt'
checkpoint_path = os.path.join(root, 'pre-trained-models', model_file)
...
class TestResidualLayer(unittest.TestCase):
    ...
    def test_grad_mag_resnet18_backbone(self):
        ...
        #ResidualModel = BasicLayer(64, 64, True).cuda()
        #summary(ResidualModel, input_size=(64, 256, 256))
        """Test size of intermediate tensor from the 1st layer to the last feature map (no head yet)"""
        N = 2  # batch size
        ...
        loss_fnc = torch.nn.L1Loss().cuda()
        resnet = ResNet18().cuda()
        #summary(resnet, input_size=(3, 384, 384))
