...
class CenterNetLoss(nn.Module):
    ...
    def focal_loss(self, input, target, mask_true_center):
        ...
        pred_noncenter_prob = input[mask_non_center] # shape (N*nClass*H*W - nCenters) - TODO
        target_noncenter_prob = target[mask_non_center]  # shape (N*nClass*H*W - nCenters) - TODO
        loss_noncenter = (1-target_noncenter_prob)**self.focal_beta * (pred_noncenter_prob)**self.focal_alpha * torch.log(1-pred_noncenter_prob)  # shape (N*nClass*H*W - nCenters) - TODO

        focal_loss = -(loss_center.sum()+ loss_noncenter.sum())/nCenters  # TODO: sum all elements in loss_center & in loss_noncenter, then divide by nCenters
        return focal_loss
    ...
    def forward(self, input, target):
        ...
        for b_idx in range(input.size(0)):  # iterate through each image in the mini-batch
            ...
            collapsed_mask_not_center = torch.logical_not(collapsed_mask_true_center)
            target_regress = input[b_idx, self.nClass:, collapsed_mask_not_center] # shape (4, nCenters_for_this_image) - TODO
