...
def idx1d_to_indices3d(idx_1d, nRows, nCols):
    ...
    """
    col_idx = torch.tensor([int(i%nCols) for i in idx_1d], dtype=torch.long)  # shape (nIndices) - TODO
    row_idx = torch.tensor([int((i - nRows*nCols*int(i/(nRows*nCols)))/nCols) for i in idx_1d],  dtype=torch.long)  # shape (nIndices) - TODO
    ch_idx = torch.tensor([int(i/(nRows*nCols)) for i in idx_1d], dtype=torch.long)  # shape (nIndices) - TODO
...
def nms_heat_map(heatmap):
    ...
    """
    heat_max = F.max_pool2d(heatmap, kernel_size=3, stride=1, padding=1)  # (N, nClasses, H, W) - TODO: declare kernel_size, stride, padding
...
def get_topK_peaks(heatmaps, topK=100):
    ...
    # take the topK peaks
    indices1d = indices1d[:topK]  # shape (topK) - TODO: keep the first `topK` elements in indices1d
    # store class probability (i.e. score) of each peak
    score = heat_sorted[:topK]  # shape (topK) - TODO: keep the first `topK` elements in heat_sorted
    ...
    # print('chs = {}, ys = {},  xs = {}'.format(chs, ys, xs))
...
def decode_prediction(prediction, nClasses=20, topK=100):
    ...
    heat = nms_heat_map(prediction)  # (N, nClasses, H, W) - TODO: invoke `nms_heat_map` on heatmap of `prediction`
    ...
    for im_idx in range(batch_size):
        ...
        chs, ys, xs, score = get_topK_peaks(heat[im_idx], topK=100)  # TODO

        # retrieve centers
        offset_x = prediction[im_idx, -2, ys, xs]  # (topK)
        offset_y = prediction[im_idx, -1, ys, xs]  # (topK) - TODO: similar to offset_x, extract offset_y from `prediction`
        ...
        center = torch.cat([(xs + offset_x).unsqueeze(1), (ys + offset_y).unsqueeze(1)], dim=-1)  # (topK, 2) TODO
        # Hint: refer to the creation of `this_im_boxes`
        # print('center = \n', center)

        # retrieve size
        w = prediction[im_idx, -4, ys, xs]  # (topK) - TODO: similar to offset_x, extract w from `prediction`
        h = prediction[im_idx, -3, ys, xs]  # (topK) - TODO: similar to offset_x, extract h from `prediction`
        # concatenate boxes
        # print('w = \n', w.shape)
        # print('h = \n', h.shape)
        # print('chs = \n', chs.shape)
