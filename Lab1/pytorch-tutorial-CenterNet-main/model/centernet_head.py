...
def conv_block(inC, midC, outC):
    ...
    return nn.Sequential(OrderedDict([
        ('conv1', nn.Conv2d(inC, midC, kernel_size=3,
                            stride=1, padding=1, bias=True)),  # TODO: declare kernel_size, stride, padding
        ('relu', nn.ReLU()),
        ('conv2', nn.Conv2d(midC, outC, kernel_size=1,
                            stride=1, padding=0, bias=True))  # TODO: declare kernel_size, stride, padding
    ]))
...
class CenterNetHead(nn.Module):
    ...
    def __init__(self, inC, nClasses, midC):
        ...
        self.heatmap = conv_block(inC, midC, 20)  # TODO: declare the outC
        self.wh = conv_block(inC, midC, 2)  # TODO: declare the outC
        self.offset = conv_block(inC, midC, 2)  # TODO: declare the outC
    ...
    def forward(self, input):
        ...
        """
        hm = self.heatmap(input)  # shape (N, nClasses, H, W) - TODO: declare input to self.heatmap()
        wh = self.wh(input)  # shape (N, 2, H, W) - TODO: declare input to self.wh()
        offset = self.offset(input)  # shape (N, 2, H, W) - TODO: declare input to self.offset()
